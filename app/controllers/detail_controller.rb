...
class DetailController < ApplicationController
    ...
    def detail_program(name)
        ...
        @detail_pic1 = image["channel"]["item"][1]["image"]
        @detail_pic2 = image["channel"]["item"][2]["image"]
=begin
        url2 = 'https://apis.daum.net/local/v1/search/keyword.json?apikey=e39c413113cbc67c3c832186c1480624&query='
        url2 += '카카오프렌즈'
        
        
        url2 = url2.force_encoding('binary')
        url2=WEBrick::HTTPUtils.escape(url2)
        map = JSON.parse(open(url2).read)
        puts map

        @longmap = map["channel"]["item"][0]["longitude"]
        @latmap = map["channel"]["item"][0]["latitude"]
=end        
